...
static int board_ATK_led_init (int which) /* 初始化LED, which-哪个LED */       
{
    if (!CCM_CCGR1)
    {
...
static int board_ATK_led_ctl (int which, char status) /* 控制LED, which-哪个LED, status:1-亮,0-灭 */
{
...
static struct led_operations board_ATK_led_opr = {
    .num  = 4,
    .init = board_ATK_led_init,
    .ctl  = board_ATK_led_ctl,
};

struct led_operations *get_board_led_opr(void)
{
    return &board_ATK_led_opr;
}
